(() => {
  ...
  function displaySeriesTitle(talk) {
    return String(talk?.title || "").replace(/\s*\(\s*\d+\s+videos?\s*\)\s*$/i, "").trim();
  }

  function displayTalkTitle(talk) {
    return store.language === "en" && talk?.titleEn ? talk.titleEn : talk?.title || "";
  }
  ...
  function countLabel(count, englishSingular, englishPlural, hindiLabel) {
    return store.language === "en" ? `${count} ${count === 1 ? englishSingular : englishPlural}` : `${count} ${hindiLabel}`;
  }
  ...
  function rowPresentation(talk) {
    ...
    const details = [];
    const durationLabel = talk?.duration?.display || (duration ? formatTime(duration) : "");
    if (durationLabel) details.push(durationLabel);
    if (status === "listened") details.push(store.language === "en" ? "Listened" : "सुना हुआ");
    if (status === "partial" && remaining) details.push(store.language === "en" ? `${formatTime(remaining)} remaining` : `${formatTime(remaining)} शेष`);
  ...
  function renderCategories() {
    ...
    grid.innerHTML = categories.map((category) => {
      ...
      const label = category.key === "Satsang Ansh" ? countLabel(count, "excerpt", "excerpts", "अंश") : category.key === "Bhajan" ? (store.language === "en" ? `${count} available` : `${count} उपलब्ध`) : countLabel(count, "collection", "collections", "संग्रह");
      return `<button class="category-card" type="button" data-category="${escapeHtml(category.key)}">
        <span><h3>${store.language === "en" ? category.en : category.hi}</h3><p>${label}</p></span><span class="row-chevron" aria-hidden="true">›</span>
      </button>`;
    }).join("");
  }
  ...
  function seriesRow(talk) {
    ...
    return `<button class="series-row" type="button" data-series="${escapeHtml(id)}" data-talk-id="${escapeHtml(id)}">
      <span class="series-row-copy"><strong>${escapeHtml(displaySeriesTitle(talk))}</strong><span>${escapeHtml(details)}</span>${count ? `<span class="series-count">${countLabel(count, "satsang", "satsangs", "सत्संग")}</span>` : ""}</span>
  ...
  function talkCard(talk, list, index) {
    ...
    return `<button class="simple-talk-row ${isSeries ? "" : `is-${state.status}${state.isCurrent ? " is-current" : ""}`}" type="button" ${action} data-talk-id="${escapeHtml(id)}">
      <span><strong>${!isSeries && state.status === "listened" ? "✓ " : !isSeries && state.status === "partial" ? "▶ " : ""}${escapeHtml(isSeries ? displaySeriesTitle(talk) : displayTalkTitle(talk))}</strong><small>${escapeHtml([categoryLabel(talk.category), metaText(talk)].join(" · "))}</small>${!isSeries && state.details ? `<small class="useful-status">${escapeHtml(state.details)}</small>` : ""}${!isSeries && state.hasManan ? `<small class="row-manan">✎ ${store.language === "en" ? "Manan" : "मनन"}</small>` : ""}${!isSeries && state.ratio ? `<span class="row-progress" aria-hidden="true"><i style="width:${state.ratio}%"></i></span>` : ""}</span>
  ...
  function renderCatalogue() {
    ...
    $("#talkGrid").innerHTML = activeCategory === "Satsang Ansh" ? anshDurationGroups.map((group) => {
      ...
      return `<button class="series-row" type="button" data-ansh-duration="${group.key}"><span class="series-row-copy"><strong>${escapeHtml(anshDurationLabel(group))}</strong><span class="series-count">${countLabel(count, "satsang", "satsangs", "सत्संग")}</span></span><span class="row-chevron" aria-hidden="true">›</span></button>`;
      ...
      return `<button class="series-row" type="button" data-bhajan-group="${group.key}"><span class="series-row-copy"><strong>${escapeHtml(store.language === "en" ? group.en : group.hi)}</strong><span>${store.language === "en" ? `${count} available` : `${count} उपलब्ध`}</span></span><span class="row-chevron" aria-hidden="true">›</span></button>`;
  ...
  function renderEpisodes(series, videos) {
    ...
    $("#episodeList").innerHTML = episodes.length ? episodes.map((episode, index) => {
      ...
      return `<button class="episode-row is-${state.status}${state.isCurrent ? " is-current" : ""}" type="button" data-talk-id="${escapeHtml(id)}" data-play="${escapeHtml(id)}">
        <span class="episode-copy"><strong>${state.status === "listened" ? "✓ " : state.status === "partial" ? "▶ " : ""}${escapeHtml(episode.title || `${store.language === "en" ? "Satsang" : "सत्संग"} ${index + 1}`)}</strong>${state.details ? `<span>${escapeHtml(state.details)}</span>` : ""}${state.hasManan ? `<small class="row-manan">✎ ${store.language === "en" ? "Manan" : "मनन"}</small>` : ""}${state.ratio ? `<span class="row-progress" aria-hidden="true"><i style="width:${state.ratio}%"></i></span>` : ""}</span>
  ...
  function openAnshDuration(key) {
    ...
    $("#anshTalkList").innerHTML = items.map((talk) => {
      const id = talkId(talk);
      const state = rowPresentation(talk);
      return `<button class="episode-row is-${state.status}${state.isCurrent ? " is-current" : ""}" type="button" data-talk-id="${escapeHtml(id)}" data-play="${escapeHtml(id)}"><span class="episode-copy"><strong>${state.status === "listened" ? "✓ " : state.status === "partial" ? "▶ " : ""}${escapeHtml(talk.title)}</strong>${state.details ? `<span>${escapeHtml(state.details)}</span>` : ""}${state.hasManan ? `<small class="row-manan">✎ ${store.language === "en" ? "Manan" : "मनन"}</small>` : ""}${state.ratio ? `<span class="row-progress" aria-hidden="true"><i style="width:${state.ratio}%"></i></span>` : ""}</span><span class="episode-play" aria-hidden="true">${state.status === "listened" ? "✓" : "▶"}</span></button>`;
    }).join("");
    location.hash = "ansh";
  }
  ...
  function searchTalks(query) {
    ...
    return unique.filter((talk) => {
      const haystack = normalize([talk.title, talk.titleEn, talk.parentTitle, talk.location, talk.year, talk.category, categoryLabel(talk.category)].filter(Boolean).join(" "));
  ...
  function renderHomePersonalization() {
    ...
    const latestPartial = history.find((talk) => statusOf(talk) === "partial");
    ...
    if (latestPartial) {
      ...
      $("#continueCard").innerHTML = `<div class="continue-label">${store.language === "en" ? "Where you paused" : "जहाँ छोड़ा था"}</div><div class="continue-card"><button class="continue-play" data-play="${escapeHtml(talkId(latestPartial))}" type="button" aria-label="${store.language === "en" ? "Continue listening" : "वहीं से जारी रखें"}">▶</button><div class="continue-copy"><h3>${escapeHtml(displayTalkTitle(latestPartial))}</h3><p>${duration && seconds ? `${escapeHtml(formatTime(Math.max(0, duration - seconds)))} ${store.language === "en" ? "remaining" : "शेष"} · ` : ""}${escapeHtml(metaText(latestPartial))}</p>${ratio ? `<span class="row-progress" aria-hidden="true"><i style="width:${ratio}%"></i></span>` : ""}</div><button class="continue-action" data-play="${escapeHtml(talkId(latestPartial))}" type="button">${store.language === "en" ? "Continue from there" : "वहीं से जारी रखें"}</button></div>`;
    }
  }
      ...
      $("#libraryGrid").innerHTML = [...groups.values()].map((group) => `<section class="manan-group">
        ...
        ${group.talks.map((talk) => {
          ...
          return `<article class="manan-talk" data-talk-id="${escapeHtml(id)}"><h3>${escapeHtml(talk.title)}</h3><p>${escapeHtml(metaText(talk))}</p>
            ${notes.map((note) => `<div class="manan-note">${Number.isFinite(note.seconds) ? `<button type="button" data-seek="${note.seconds}" data-talk="${escapeHtml(id)}">▶ ${formatTime(note.seconds)} से सुनें</button>` : ""}<p>${escapeHtml(note.text)}</p><button class="delete-manan" type="button" data-delete-manan="${escapeHtml(note.id)}" data-talk="${escapeHtml(id)}" aria-label="मनन हटाएँ">हटाएँ</button></div>`).join("")}
  ...
  function renderTalkManan() {
    ...
    $("#talkMananNotes").innerHTML = notes.length ? notes.map((note) => `<article class="manan-note">
      ${Number.isFinite(note.seconds) ? `<button type="button" data-seek="${note.seconds}" data-talk="${escapeHtml(id)}">▶ ${formatTime(note.seconds)} से सुनें</button>` : ""}
  ...
  function updateMiniPlayer() {
    ...
    const talk = playerList[playerIndex];
    const onPlayerPage = location.hash.split("/")[0] === "#player";
    mini.hidden = !talk || onPlayerPage;
    document.body.classList.toggle("has-mini-player", Boolean(talk && !onPlayerPage));
    if (!talk) return;
    let seconds = Number(store.progress[talkId(talk)]?.seconds || 0);
    let duration = durationSeconds(talk);
    let playing = false;
    try {
      seconds = Math.floor(activeMainPlayer?.getCurrentTime?.() || seconds);
      duration = Math.floor(activeMainPlayer?.getDuration?.() || duration);
      playing = activeMainPlayer?.getPlayerState?.() === window.YT?.PlayerState?.PLAYING;
    } catch {}
    $("#miniPlayerTitle").textContent = displayTalkTitle(talk);
    $("#miniPlayerTime").textContent = `${formatTime(seconds)}${duration ? ` / ${formatTime(duration)}` : ""}`;
    $("#miniPlayerProgress").style.width = duration ? `${Math.min(100, seconds / duration * 100)}%` : "0%";
    $("#miniPlayerToggle").firstElementChild.textContent = playing ? "Ⅱ" : "▶";
    $("#miniPlayerToggle").setAttribute("aria-label", playing ? (store.language === "en" ? "Pause" : "रोकें") : (store.language === "en" ? "Play" : "चलाएँ"));
  }
  ...
  function openTalk(id, list = null) {
    ...
    const talk = playerList[playerIndex];
    if (!talk) return;
    const current = store.progress[id];
    if (!current || current.status === "new") store.progress[id] = { status: "partial", updatedAt: Date.now() };
    updateHistory(talk);
    saveStore();

    if (location.hash !== "#player") playerReturnHash = location.hash || "#home";
    $("#playerCategory").textContent = categoryLabel(talk.category);
    $("#playerBackContext").textContent = talk.parentTitle ? displaySeriesTitle({ title: talk.parentTitle }) : (store.language === "en" ? "Satsang" : "सत्संग");
    $("#playerTitle").textContent = displayTalkTitle(talk);
    $("#playerMeta").textContent = metaText(talk);
    $("#openYouTube").href = talk.youtubeUrl;
    $("#privateNote").value = "";
    pendingMananSeconds = null;
    $("#selectedTimestamp").hidden = true;
    $(".private-note").hidden = false;
    renderTalkManan();
    updatePlayerControls();
    location.hash = "player";
    startMainPlayer(talk.youtubeUrl);
    renderAllDynamic();
    if (activeSeries && episodeCache.has(playlistId(activeSeries))) renderEpisodes(activeSeries, episodeCache.get(playlistId(activeSeries)));
  }
  ...
  function updatePlayerControls() {
    ...
    $("#markListened").textContent = listened ? (store.language === "en" ? "↶ Mark partial" : "↶ फिर आंशिक करें") : (store.language === "en" ? "✓ Mark listened" : "✓ सुना हुआ");
    $("#openYouTube").textContent = store.language === "en" ? "Watch on YouTube ↗" : "YouTube पर देखें ↗";
  ...
  function route() {
    ...
    if (valid === "player" && playerIndex >= 0 && !activeMainPlayer && !$(".video-frame iframe")) setTimeout(() => startMainPlayer(playerList[playerIndex].youtubeUrl), 0);
  ...
  function wireEvents() {
    ...
    $("#miniPlayerOpen").addEventListener("click", () => { location.hash = "player"; });
    $("#miniPlayerToggle").addEventListener("click", () => {
      if (!activeMainPlayer?.getPlayerState) { location.hash = "player"; return; }
      const playing = activeMainPlayer.getPlayerState() === window.YT.PlayerState.PLAYING;
      if (playing) activeMainPlayer.pauseVideo(); else activeMainPlayer.playVideo();
      setTimeout(updateMiniPlayer, 80);
    });
